# Baldev Purohit

**Production Engineer (2.8+ years)** transitioning into **IT & Data Roles**.

Currently learning **Python**, **Generative AI**, and Data Analytics to combine my manufacturing domain knowledge with technical skills.

### 🛠️ Currently Learning
- The Joy of Computing using Python (NPTEL - IIT Ropar)
- Generative AI for Beginners (Jenny's Lectures)
- Data Analysis & MIS Reporting

### 📂 Projects

#### 1. Simple Calculator
- A basic Python calculator supporting addition, subtraction, multiplication, and division.
- Demonstrates use of functions (`def`), conditional statements (`if-elif-else`), and error handling.
- [View Code → simple_calculator.py](simple_calculator.py)

### 🧰 Skills
- Python (Beginner)
- Generative AI
- MIS Reporting & Excel (Advanced)
- SAP | PLM | DMS
- Root Cause Analysis & Process Improvement

### Education
- **B.E. Production Engineering**  
  Shantilal Shah Engineering College, Bhavnagar (GTU) — 2021

---

**Open to Work** | Looking for **MIS Executive, Junior Data Analyst, Python Support, IT Support** roles.

📍 Ahmedabad, Gujarat, India  
📧 purohitbaldev99@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/baldev-purohit)
